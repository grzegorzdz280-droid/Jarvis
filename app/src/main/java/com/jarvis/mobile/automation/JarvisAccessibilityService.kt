...
class JarvisAccessibilityService : AccessibilityService() {
    ...
    fun screenSummary(): String {
        ...
        val out=StringBuilder(); walk(root,out,0); return out.toString().take(8000)
    }
    private fun walk(n: AccessibilityNodeInfo, out:StringBuilder, depth:Int) {
        ...
        if(!text.isNullOrBlank()) out.append("[${n.className}] \"$text\" $at$checkInfo\n")
        else if((n.isCheckable||n.isClickable)&&rect.width()>0&&rect.height()>0) out.append("[${n.className}] (no label) $at$checkInfo\n")
