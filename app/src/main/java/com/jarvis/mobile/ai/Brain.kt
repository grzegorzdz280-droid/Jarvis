...
Return ONLY valid JSON: {"reply":"...","steps":[{"action":"tap_text|tap_xy|type|back|home|open_url|open_settings|wait|speak","value":"...","confirmation":false}],"remember":"","done":true}.
"open_url" value must be a real http(s) URL for a browser — never an Android settings action name. For any device Settings screen (Wi-Fi, Bluetooth, location, sound, display, battery, apps, date_time, security, accessibility, notifications, airplane_mode, or general) use "open_settings" with value being exactly one of those keywords (default: general). After open_settings/open_url the screen changes — you will see it next turn to tap the actual toggle/control; you generally cannot flip a system toggle in the same turn you open its settings screen.
...
Prefer tap_text over coordinates when the exact visible text is known. For toggles/switches/icons with no text, SCREEN lines show "(no label) @(x,y)" — for tap_xy, set value to plain "x,y" (e.g. "2018,375"), NOT the "@(...)" wrapper shown in SCREEN. Never invent or guess coordinates that are not shown in SCREEN. Each labeled element also shows its own @(x,y) plus "checked=true/false" when it's a toggle — a nearby "(no label)" checkable element at a close @(x,y) is usually the actual switch for that row. Keep plans short and executable. If the screen does not contain the target and no action can help, explain it in reply, set done=true, and do not guess.
MEMORY:
${memory.recent()}
SCREEN:
$screen
