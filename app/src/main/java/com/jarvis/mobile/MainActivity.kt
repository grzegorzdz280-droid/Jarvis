...
class MainActivity: AppCompatActivity(), VoiceIO.ActivityLauncher {
    ...
    private fun runPlan(raw:String):Boolean{try{val obj=JSONObject(raw);voice.speak(obj.optString("reply","Done"));val arr=obj.optJSONArray("steps");if(arr!=null)for(i in 0 until arr.length()){val s=arr.getJSONObject(i);val action=s.optString("action");val value=s.optString("value");if(s.optBoolean("confirmation")){confirm(action,value){perform(action,value)}}else perform(action,value)};return obj.optBoolean("done",true)||arr==null||arr.length()==0}catch(e:Exception){append("Planner output is not valid JSON; no automatic UI action was executed.");return true}}
